...
save(greedy.local, local.local, anneal.anneal, file = "~/ws-r/nsp/example_1_data.RData")
...
init1.expanded <- init1[rep(1:4, times = times.vec), ]
rownames(init1.expanded) <- rownames(bns1[[1]]$solution)

heatmap(init1.expanded, Rowv = NA, Colv = NA, col = cols3(10), main = "Worker Preferences as Expressed in 'init1'",
        xlab = "Workers")
dev.new()
heatmap(bns2[[1]]$solution * 1, Rowv = NA, Colv = NA, col = cols3(2), main = "Example of Final Timetable",
        xlab = "Workers")
mtext("Shift Names", 4, pos = 3)
